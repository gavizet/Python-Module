...
def ft_progress(lst):
    ...
    for element in lst:
        current_iter += 1
        percentage_completed = current_iter / total_iterations
        ...
        # Time it takes to do one full loop of our generator function
        operation_time = elapsed_time / current_iter
        # Time left before end of the progress bar's execution
        eta = operation_time * (total_iterations - current_iter)
        # Print f-string to stdout with a carriage return
        print(
            f"ETA: {eta:.2f}s"
            f"[{percentage_completed:.2%}]"
            f"[{bar_done:s}>{bar_left:s}]"
            f"{current_iter:d}/{total_iterations:d}"
